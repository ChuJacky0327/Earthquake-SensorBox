    ...
    if __name__ == "__main__":
        ...
        if abs(axes['x'] - original['x']) >= 0.1:
            ...
            if (I>2) :
                ...
                client = connect_mqtt()
                #client.loop_start()
                text = 'earthquake_intensity : '+str(I)+' | turn off greenLED'
                publish(client, text)
                if (I>3):
                    ser.write(b'earthquake3\n')
                    #time.sleep(1)
                    print('3_OK')
                    client = connect_mqtt()
                    #client.loop_start()
                    text1 = 'turn off yellowLED'
                    ...
                    if (I>4):
                        ...
                        client = connect_mqtt()
                        #client.loop_start()
                        text2 = 'turn off yellowLED'
                        publish(client, text2)
                        if (I>5):
                            ser.write(b'earthquake5\n')
                            #time.sleep(1)
                            print('5_OK')
                            client = connect_mqtt()
                            #client.loop_start()
                            text3 = 'turn off redLED'
        ...
        elif abs(axes['y'] - original['y']) >= 0.1:  
            print( "   x = %.3fG" % ( axes['x'] - original['x']))
            print( "   y = %.3fG" % ( axes['y'] - original['y']))
            print ("   z = %.3fG" % ( axes['z'] - original['z']))
            ax = axes['x'] - original['x']
        ...
        elif abs(axes['z'] - original['z']) >= 0.1:
            print ("   x = %.3fG" % ( axes['x'] - original['x']))
            print ("   y = %.3fG" % ( axes['y'] - original['y']))
            print ("   z = %.3fG" % ( axes['z'] - original['z']))
            ax = axes['x'] - original['x']
